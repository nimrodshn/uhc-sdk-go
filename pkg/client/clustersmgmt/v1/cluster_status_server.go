...
type ClusterStatusServer interface {
	...
	//
	//
	Get(request *ClusterStatusGetServerRequest, response *ClusterStatusGetServerResponse) error
}
...
func (r *ClusterStatusGetServerRequest) GetContext() (value context.Context, ok bool) {
	ok = r != nil && r.ctx != nil
	if ok {
		value = r.ctx
	}
